...
test('CLI generates fake csproj and fake sln in a clean temp workspace', () => {
  ...
  try {
    ...
    const slnPath = path.join(tempDir, 'sample-workspace.sln');
    ...
    fs.writeFileSync(path.join(websiteBinDir, 'AuditTrail.dll'), '');
    fs.writeFileSync(path.join(websiteBinDir, 'Newtonsoft.Json.dll'), '');

    const result = spawnSync(
      process.execPath,
      [cliPath, slnPath, '--mode', 'link', '--outDir', 'intellisense-output'],
      {
        cwd: repoRoot,
        encoding: 'utf8',
      }
    ...
    const fakeDir = path.join(tempDir, 'intellisense-output', 'Web');
    ...
    assert.ok(fakeSln.includes('"..\\..\\..\\..\\SharedComponents\\AuditTrail\\AuditTrail.csproj"'));
  } finally {
